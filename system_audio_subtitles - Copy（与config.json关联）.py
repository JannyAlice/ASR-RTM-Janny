...
class SubtitleWindow(QMainWindow):
    ...
    def create_menu_bar(self):
        ...
        # 英文模型子菜单
        en_model_menu = QMenu('英文 (En->Cn)', self)
        ...
        # 中文模型子菜单
        cn_model_menu = QMenu('中文 (Cn->En)', self)
        ...
        # 添加模型动作到子菜单
        en_model_menu.addAction(self.en_small_action)
        en_model_menu.addAction(self.en_medium_action)
        en_model_menu.addAction(self.en_large_action)
        
        cn_model_menu.addAction(self.cn_small_action)
        cn_model_menu.addAction(self.cn_medium_action)
        cn_model_menu.addAction(self.cn_large_action)
        
        # 添加子菜单到模型菜单
        model_menu.addMenu(en_model_menu)
        model_menu.addMenu(cn_model_menu)
    ...
    def start_transcription(self):
        ...
        if not self.is_system_audio:
            ...
            try:
                ...
                print(f"获取文件时长错误: {e}")
                self.signals.new_text.emit(f"获取文件时长错误: {e}")
                self.stop_transcription()
                return
        else:
            # 系统音频模式
    ...
    def _update_ui_after_finish(self):
        ...
        try:
            ...
            # 显示完成提示
            current_text = self.subtitle_label.text()
            completion_message = f"\n\n转录已完成！文件已保存到: {self.output_file}"
            self.subtitle_label.setText(current_text + completion_message)
            ...
            print(f"转录完成，文件已保存到: {self.output_file}")
        except Exception as e:
