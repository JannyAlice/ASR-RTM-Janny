...
def test_sherpa_2023_06_26_persistent():
    """测试 sherpa-onnx-streaming-zipformer-en-2023-06-26 基本模型的在线转录功能（持久流方式）"""
    print("=" * 80)
    print("测试 sherpa-onnx-streaming-zipformer-en-2023-06-26 基本模型的在线转录功能（持久流方式）")
    print("=" * 80)
    ...
    transcript_file = os.path.join(project_root, "transcripts", f"transcript_2023_06_26_persistent_{timestamp}.txt")
    os.makedirs(os.path.dirname(transcript_file), exist_ok=True)
    
    # 写入文件头
    with open(transcript_file, "w", encoding="utf-8") as f:
        f.write(f"# Sherpa-ONNX 2023-06-26 基本模型转录结果（持久流方式）\n")
...
if __name__ == "__main__":
    test_sherpa_2023_06_26_persistent()
