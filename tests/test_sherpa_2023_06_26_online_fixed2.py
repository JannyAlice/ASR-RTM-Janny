"""
测试 sherpa-onnx-streaming-zipformer-en-2023-06-26 模型的在线转录功能（改进版）
"""
import os
import sys
import time
...
def test_sherpa_2023_06_26_online_fixed2():
    """测试 sherpa-onnx-streaming-zipformer-en-2023-06-26 模型的在线转录功能（改进版）"""
    print("=" * 80)
    print("测试 sherpa-onnx-streaming-zipformer-en-2023-06-26 模型的在线转录功能（改进版）")
    print("=" * 80)
    ...
    encoder_file = "encoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx" if use_int8 else "encoder-epoch-99-avg-1-chunk-16-left-128.onnx"
    decoder_file = "decoder-epoch-99-avg-1-chunk-16-left-128.int8.onnx" if use_int8 else "decoder-epoch-99-avg-1-chunk-16-left-128.onnx"
    joiner_file = "joiner-epoch-99-avg-1-chunk-16-left-128.int8.onnx" if use_int8 else "joiner-epoch-99-avg-1-chunk-16-left-128.onnx"
    ...
    # 设置参数
    sample_rate = 16000
    buffer_size = 8000  # 增加缓冲区大小到500ms，提高识别准确率
        ...
        with sc.get_microphone(id=str(default_device.id), include_loopback=True).recorder(samplerate=sample_rate) as mic:
            ...
            try:
                ...
                try:
                    ...
                    if final_text and final_text != last_result:
                        # 格式化文本
                        if final_text:
...
if __name__ == "__main__":
    test_sherpa_2023_06_26_online_fixed2()
